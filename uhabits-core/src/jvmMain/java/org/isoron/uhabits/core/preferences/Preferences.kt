...
open class Preferences(private val storage: Storage) {
    ...
    var isScoreWheelEnabled: Boolean
        get() = storage.getBoolean("pref_show_score_wheel", false)
        set(enabled) {
            storage.putBoolean("pref_show_score_wheel", enabled)
        }

    var isStreakViewEnabled: Boolean
        get() = storage.getBoolean("pref_show_streak_view", true)
        set(enabled) {
            storage.putBoolean("pref_show_streak_view", enabled)
        }
